...
Dejar un repositorio de equipo que otra persona pueda clonar, instalar y verificar sin conocer tu computadora. La entrega de esta semana tiene cuatro resultados:
...
3. El starter compila y `npm run verify`, `npm test` y `npm run build` terminan con código 0.
...
2. Trabaja en el repositorio privado común del equipo.
...
8. Ejecuta `npm run verify`, `npm test` y `npm run build`. Si tienes Bash disponible, ejecuta también `bash public-tests/check.sh`.
